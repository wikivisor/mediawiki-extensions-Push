...
$(function () {
  ...
  $("#divIncTemplates")
    .on(
      ...
      function () {
        ...
        $txtTemplateList.fadeTo(isHidden ? "slow" : "fast", 1);
      },
      function () {
  ...
  $("#divIncFiles")
    ...
    .on(
      ...
      function () {
        ...
        $txtFileList.fadeTo(isHidden ? "slow" : "fast", 1);
      },
      function () {
          ...
          if (Object.prototype.hasOwnProperty.call(files, i)) {
            ...
            $txtFileList.append(
              $("<a>")
                .attr(
                  "href",
                  mw.config.get("wgPushIndexPath") + "?title=" + files[i],
                )
                .text(files[i]),
            );
          }
  ...
  function displayTargetConflictStatus(targetId) {
    ...
    if ($("#checkIncTemplates").prop("checked")) {
      ...
      for (remotePageId in targetData[targetId].existingPages) {
        if (
          Object.prototype.hasOwnProperty.call(
            targetData[targetId].existingPages,
            remotePageId,
          )
        ) {
          if (targetData[targetId].existingPages[remotePageId].ns === 10) {
            // Add the template, but get rid of the namespace prefix first.
            overrideTemplates.push(
              targetData[targetId].existingPages[remotePageId].title.split(
                ":",
                2,
              )[1],
            );
    ...
    new mw.Api()
      ...
      .done(function (data) {
        if (data.error) {
          ...
          handleError(sender, {
            info: mw.msg("push-err-captacha", targetName),
          });
        } else {
  ...
  function setButtonToImgPush(button, pages, targetUrl, targetName) {
    var images = mw.config
      .get("wgPushPageFiles")
      .concat(mw.config.get("wgPushTemplateFiles"));

    if (
      images.length > 0 &&
      $checkIncFiles.length !== 0 &&
      $checkIncFiles.prop("checked")
    ) {
    ...
    new mw.Api()
      ...
      .done(function (data) {
        ...
        for (const key in data) {
          ...
          if (key === "error") continue; // already handled

          const item = data[key];
          if (!item) continue; // <‑‑ guard against null/undefined

          if (item.error) {
            if (item.error.code === "fileexists-no-change") {
              break; // silently skip
            }
            item.error.info = mw.msg("push-tab-err-filepush", item.error.info);
            handleError(sender, item.error);
            fail = true;
            break;
          } else if (!item.upload) {
            item.error = { code: "unknown", info: "" };
            item.error.info = mw.msg("push-tab-err-filepush-unknown");
            handleError(sender, item.error);
            fail = true;
            break;
          }
      ...
      .fail(function (jqXHR) {
        // ← corrected
        const json = jqXHR.responseJSON || JSON.parse(jqXHR.responseText);
        if (!json.error) {
          json.error = { code: "unknown", info: "Unknown error" };
        }
        json.error.info = mw.msg("push-tab-err-filepush", json.error.info);
        handleError(sender, json.error);
      });
  }
  ...
  function reEnableButton(button) {
    ...
    getRemoteArticleInfo(
      $(button).attr("targetid"),
      $(button).attr("pushtarget"),
    );
